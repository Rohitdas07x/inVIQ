    ...
    <div className="flex flex-col min-h-full bg-[#F8FAFC]">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-slate-200 px-6 py-3.5 shadow-2xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Stock Acquisition &amp; Ingest</h2>
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-semibold rounded-none border border-slate-300 transition-colors cursor-pointer"
            >
      ...
      <div className="p-6 md:p-8 max-w-7xl mx-auto w-full space-y-6 flex-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-200 p-4 rounded-none">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Manifests</span>
              <FileSpreadsheet className="w-4 h-4 text-blue-600" />
            </div>
            <p className="text-2xl font-bold text-slate-900 mt-2">{uploads.length}</p>
            <p className="text-[11px] text-slate-400 mt-1">Delivery batches submitted</p>
          </div>

          <div className="bg-white border border-slate-200 p-4 rounded-none">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Stock Units Ingested</span>
              <Package className="w-4 h-4 text-emerald-600" />
            </div>
            <p className="text-2xl font-bold text-slate-900 mt-2">{totalRowsProcessed.toLocaleString()}</p>
            <p className="text-[11px] text-emerald-600 font-medium mt-1">Auto-credited to inventory</p>
          </div>

          <div className="bg-white border border-slate-200 p-4 rounded-none">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Ingest Success Rate</span>
              <CheckCircle className="w-4 h-4 text-indigo-600" />
            </div>
            <p className="text-2xl font-bold text-slate-900 mt-2">{successRate}%</p>
            <p className="text-[11px] text-slate-400 mt-1">{successUploadsCount} successful uploads</p>
          </div>

          <div className="bg-white border border-slate-200 p-4 rounded-none">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Active Branches</span>
              <Building2 className="w-4 h-4 text-slate-600" />
            </div>
            <p className="text-2xl font-bold text-slate-900 mt-2">{locations.length}</p>
            <p className="text-[11px] text-slate-400 mt-1">Ready for intake</p>
          </div>
        </div>

        {/* ── Notification Banners ────────────────────────────────────────── */}
        {error && (
          <div className="p-4 text-xs font-medium border border-red-200 bg-red-50 text-red-800 flex items-center justify-between">
        ...
        {success && (
          <div className="p-4 text-xs font-medium border border-emerald-200 bg-emerald-50 text-emerald-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-5 bg-white border border-slate-200 p-5 rounded-none space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-blue-50 text-blue-600 rounded-none border border-blue-100">
                  <Upload size={16} />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Upload Delivery Manifest</h3>
                  <p className="text-[11px] text-slate-500">Ingest wholesale invoices directly into stock</p>
                </div>
              </div>
            </div>

            {/* Template Download Prompt */}
            <div className="bg-slate-50 border border-slate-200 p-3.5 flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold text-slate-800">Need the standard format?</p>
                <p className="text-[11px] text-slate-500 mt-0.5">Use our template with Item Name, Batch, and Qty columns.</p>
              </div>
              <button
                type="button"
                onClick={handleDownloadTemplate}
                className="shrink-0 flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-700 bg-white border border-slate-300 px-2.5 py-1.5 hover:bg-slate-50 transition-colors cursor-pointer"
              >
                <Download size={12} />
                <span>Template</span>
              </button>
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  Receiving Branch / Store <span className="text-red-500">*</span>
                </label>
                <div className="relative flex items-center">
                  ...
                  <select
                    ...
                    className="w-full text-xs font-medium bg-slate-50 border border-slate-300 text-slate-800 rounded-none pl-9 pr-8 py-2.5 hover:bg-white focus:outline-none focus:ring-1 focus:ring-blue-600 cursor-pointer"
                  >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  Delivery Manifest File <span className="text-red-500">*</span>
                </label>
                <input
                  id="file-upload"
                  type="file"
                ...
                >
                  {file ? (
                    <>
                      ...
                      <p className="text-xs font-bold text-emerald-800 text-center truncate max-w-xs">{file.name}</p>
                    ...
                    <>
                      <div className="p-2.5 bg-white text-slate-400 border border-slate-200 rounded-full mb-2">
                        <Upload size={18} />
                      </div>
                      <p className="text-xs font-medium text-slate-700">
                        Drop Excel/CSV manifest here, or <span className="text-blue-600 font-bold">browse</span>
                      </p>
                      <p className="text-[11px] text-slate-400 mt-1">Supports .xlsx, .xls, .csv · Max 10MB</p>
                    </>
                  )}
                </label>
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white text-xs font-semibold rounded-none transition-colors shadow-2xs cursor-pointer"
              >
                {loading ? (
                  <>
          ...
          <div className="lg:col-span-7 bg-white border border-slate-200 p-5 rounded-none space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-slate-100 text-slate-700 rounded-none border border-slate-200">
                  <History size={16} />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Recent Ingest History</h3>
                  <p className="text-[11px] text-slate-500">{uploads.length} total manifest submissions</p>
                </div>
              </div>
              ...
              <div className="relative w-full sm:w-56">
                ...
                <input
                  ...
                  className="w-full text-xs bg-slate-50 border border-slate-300 rounded-none pl-8 pr-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-600"
                />
              </div>
            </div>
            ...
            {fetching ? (
              ...
              <div className="py-16 text-center flex flex-col items-center justify-center border border-dashed border-slate-200 p-6">
                <FileSpreadsheet className="w-10 h-10 text-slate-300 mb-2" />
                <h4 className="text-xs font-bold text-slate-800">No Manifests Found</h4>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs">
                    {filteredUploads.map((upload, idx) => {
                      ...
                      return (
                        <tr key={upload.id || idx} className="hover:bg-slate-50/75 transition-colors">
                          <td className="py-3 px-3 font-medium text-slate-900 max-w-[140px] truncate" title={upload.filename}>
